...
function preAddManifest() {
	...
	$("#manifestName").val("");
	$("#weight").val("");
	$("#boxOwnerId").combobox("setValue", "1");
	...
	$("#sendCity").val("");
	$("#sendArea").val("");
	$("#sendDetailedAddress").val("");
	$("#sendDate").datebox("setValue", "");
	$("#receiveProvince").val("");
	$("#receiveCity").val("");
	$("#receiveArea").val("");
	$("#receiveDetailedAddress").val("");
...
function delManifest() {
	var manifest = $("#grid1").datagrid("getSelected");
	if (manifest == '' || manifest == null) {
		$.messager.alert("提示", "请选择要清零的货单！", "info");
	} else {
		manifestStatus = manifest.manifestStatus;
		var manifestId = manifest.manifestId;
		if (manifestStatus == MANIFEST_STATUS_1) {
			$.messager.confirm('确认', '确定清除该货单剩余箱数吗？', function(r) {
				if (r) {
					$.ajax({
						type : "post",
						...
						error : function(request, errinfo, errobject) {
							$.messager.alert("系统错误", "剩余箱数清零失败！" + errobject,
									"error");
						}
					});
				} else {
			...
			$.messager.alert("提示", "货单状态为【抢单中】才允许清空！", "info");
		}
	}
}
